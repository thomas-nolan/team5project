...
public class SettingsPage implements Screen {
   ...
   * @param uiController the controller for the UI layer
   * @param previousScreen the previous screen displayed
   */
  ...
  /**
   * Displays the background,button and slider of the menu.
   * Also initialises width and height
   */
  @Override
  public void show() {
    // Load textures
    ...
    sliderBar = new Rectangle(centerX - sliderWidth / 2f, 
        screenHeight / 2f - 50f, sliderWidth, sliderHeight);
  ...
  /**
   * This method is used to display the setting page.
   */
  public void display() {
    ...
    // Draw slider knob
    game.batch.draw(sliderKnobTexture, 
        sliderKnob.x, sliderKnob.y, sliderKnob.width, sliderKnob.height);
   ...
   * @param hovered - True if player hovers, false if not
   */
  ...
  /**
   * Updates the slider depending on the volume value (0-100).
   */
  ...
  /**
   * Adjusts the volume based on the audio level.
   * 
   ...
   * @return - True if button clicked, False if not
   */
  ...
  /**
   * Highlights the button when the player moves the cursor over it.
   * 
   * @param button the button to highlight 
   * @return a highlighted button
   */
  ...
  /**
   * Returns to the main menu from the settings screen.
   * Also disposes of textures
   */
  public void onBack() {
  ...
  /** 
   * Dispose of all textures when they are no longer needed.
   */
  @Override
  public void dispose() {
